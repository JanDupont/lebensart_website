<template>
    <DuoContentGrid bg-color="#010606" style="margin-top: 12px">
        <template #left_and_bottom>
            <div v-if="!showMap" class="öffnungszeitenMap d-flex align-center justify-center">
                ...
                <div style="position: absolute">
                    ...
                    <br />
                    <v-btn color="success" @click="showMap = !showMap">{{
                        t('OpeningHoursPage.maps.action')
                    }}</v-btn>
                </div>
            </div>
...
function isHoursEntry(value: unknown): value is HoursEntry {
    return isRecord(value) && typeof value.short === 'string' && typeof value.time === 'string';
}

function isSpecialHoursEntry(value: unknown): value is SpecialHoursEntry {
    return isRecord(value) && typeof value.label === 'string' && typeof value.time === 'string';
}

const days = computed<Record<string, HoursEntry>>(() => {
    const data = tm('OpeningHoursPage.days');
    if (!isRecord(data)) return {};

    return Object.fromEntries(
        Object.entries(data).filter((entry): entry is [string, HoursEntry] =>
            isHoursEntry(entry[1]),
        ),
    );
});

const specialHours = computed<SpecialHoursEntry[]>(() => {
    const data = tm('OpeningHoursPage.specialHours');
    return Array.isArray(data) ? data.filter(isSpecialHoursEntry) : [];
});
